...
pub fn main() void {
    ...
    while (!c.WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        if (draw_ring)
            c.DrawRing(center, inner_radius, outer_radius, start_angle, end_angle, @intFromFloat(segments), c.Fade(c.MAROON, 0.3));
        if (draw_ring_lines)
            c.DrawRingLines(center, inner_radius, outer_radius, start_angle, end_angle, @intFromFloat(segments), c.Fade(c.BLACK, 0.4));
        if (draw_circle_lines)
            c.DrawCircleSectorLines(center, outer_radius, start_angle, end_angle, @intFromFloat(segments), c.Fade(c.BLACK, 0.4));
        ...
        _ = c.GuiSliderBar(.{ .x = 600.0, .y = 240.0, .width = 120.0, .height = 20.0 }, "Segments", null, &segments, 0, 100);
